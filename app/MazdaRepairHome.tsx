...
const systems: { title: string; detail: string; icon: string; href?: string }[] = [
  { title: "Engine & Air", detail: "Air intake, PCV valves, belts & mounts", icon: "engine", href: "/cars/mazda/mazda-3/2007/2-3l/engine-air" },
      ...
      <main id="main-content">
        ...
        <section aria-labelledby="systems-heading" className="mx-auto max-w-7xl px-6 py-16 md:py-20">
          ...
          <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {systems.map(system => {
              const cardClass = `flex flex-col rounded-xl border border-neutral-200 bg-white p-6 ${cardMotion}`;
              const body = <>
                <div className="mb-7 flex items-center justify-between"><PartIcon kind={system.icon} /><span className="text-xs font-medium text-neutral-500">{system.href ? "Explore →" : "Coming soon"}</span></div>
              ...
              return system.href
                ? <Link key={system.title} href={system.href} className={cardClass}>{body}</Link>
                : <article key={system.title} className={cardClass}>{body}</article>;
            })}
          </div>
        </section>
